...
def self_play(
    ...
    if os.path.exists(rust_bin_path):
        ...
        rust_proc = subprocess.Popen(
            ["cargo", "run", "--release", "--bin", "self_play_worker"],
            cwd=os.path.join(os.path.dirname(__file__), "..", "..", "..", "src", "tricked_rs"),
        )
    ...
    try:
        ...
        with Progress(
            ...
            TimeRemainingColumn(),
            TextColumn(
                " | [yellow]Med[/]: {task.fields[med]:.1f} | [red]Max[/]: {task.fields[max]:.0f} | [magenta]Avg[/]: {task.fields[avg]:.1f}"
            ),
            console=console,
            transient=True,
        ) as progress:
            task1 = progress.add_task(
                "Hybrid MCTS Generation", total=num_games, med=0, max=0, avg=0
            )
            ...
            while completed_games < num_games:
                ...
                offset = 72
                b_np = np.frombuffer(
                    payload, dtype=np.uint64, count=b_len // 8, offset=offset
                ).reshape(length, 2)
                offset += b_len
                av_np = np.frombuffer(
                    payload, dtype=np.int32, count=av_len // 4, offset=offset
                ).reshape(length, 3)
                offset += av_len
                ...
                offset += r_len
                pol_np = np.frombuffer(
                    payload, dtype=np.float32, count=pol_len // 4, offset=offset
                ).reshape(length, 288)
                offset += pol_len
