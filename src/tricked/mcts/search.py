...
class MuZeroMCTS:
    ...
    def search(
        ...
        with torch.no_grad():
            ...
            flat_feature = tricked_engine.extract_feature(
                root_state, history, action_history, difficulty
            )
            x_t = torch.tensor(flat_feature, dtype=torch.float32, device=self.device).reshape(
                1, 20, 96
            )
