...
export function RunsSidebarListItem({
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ws = new WebSocket(`ws://127.0.0.1:8000/api/ws/runs/${run.id}/metrics`);
      ws.binaryType = "arraybuffer";

      ws.onmessage = async (event) => {
        if (!active) return;
        try {
          const { MetricHistory } = await import("@/bindings/proto/tricked");
          ...
          console.error("Sidebar metric decode error:", e);
        }
      };
  ...
  useEffect(() => {
    if (draftColor !== null) {
      const timer = setTimeout(() => {
        setRunColors((prev: any) => ({ ...prev, [run.id]: draftColor }));
        setDraftColor(null);
      }, 600);
      return () => clearTimeout(timer);
    }
  }, [draftColor, run.id, setRunColors]);

  const globalRunColor = runColors[run.id] || defaultColors[idx % defaultColors.length];
  const runColor = draftColor !== null ? draftColor : globalRunColor;
  const isColorDirty = draftColor !== null && draftColor !== globalRunColor;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <input
              ...
              className="w-3.5 h-3.5 p-0 border-0 rounded cursor-pointer bg-transparent shrink-0"
