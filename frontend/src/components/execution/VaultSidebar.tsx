...
export function VaultSidebar() {
  ...
  const flushVault = useVaultStore((state) => state.flushVault);
  const setSortBy = useVaultStore((state) => state.setSortBy);
  const setSelectedGameIndex = useVaultStore(
    (state) => state.setSelectedGameIndex,
  );
  ...
  const sortedGamesWithIndex = useMemo(() => {
    return games
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "score") diff = a.g.episode_score - b.g.episode_score;
          ...
          diff = a.g.lines_cleared - b.g.lines_cleared;
        else if (sortBy === "depth")
          diff = a.g.mcts_depth_mean - b.g.mcts_depth_mean;
        else if (sortBy === "time")
          diff = a.g.mcts_search_time_mean - b.g.mcts_search_time_mean;
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-1">
            <Button
              ...
              size="icon"
              onClick={() => {
                if (window.confirm("Are you sure you want to EMPTY ALL VAULTS entirely? This cannot be undone and deletes all games!")) {
                  useVaultStore.getState().emptyVault();
                }
              }}
              disabled={loading}
              title="Empty entire vault (Wipe Data)"
              className="h-6 w-6 bg-red-500/10 border-red-500/20 hover:bg-red-500/20 text-red-400 group"
            >
              <VscTrash className={`w-3 h-3 group-hover:scale-110 transition-transform ${loading ? "opacity-50" : ""}`} />
            </Button>
            <Button
              variant="outline"
              size="icon"
        ...
        {error ? (
          ...
          <ScrollArea className="h-full pr-2">
            <div className="flex flex-col gap-1.5">
              {sortedGamesWithIndex.map(({ g, originalIndex }) => (
                <VaultGameItem
                  key={originalIndex}
                  g={g}
                  runs={runs}
                  originalIndex={originalIndex}
                  isSelected={selectedGameIndex === originalIndex}
...
const VaultGameItem = memo(
  ({
    ...
    const matchedRun = runs.find(
      (r) =>
        r.id === g.source_run_id ||
        r.id === g.source_run_name ||
        r.name === g.source_run_name ||
        (g.source_run_name.startsWith("Training ") &&
          r.id.startsWith(g.source_run_name.replace("Training ", "")))
    );
    const displayName = matchedRun ? matchedRun.name : g.source_run_name;
    ...
    >
      <div className="flex justify-between items-center border-b border-white/5 pb-1">
        ...
        <div className="flex gap-1 items-center">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="h-4 w-4 ml-1 opacity-0 group-hover:opacity-100 hover:bg-red-500/20 text-red-500/50 hover:text-red-400 transition-opacity"
            onClick={(e) => {
              e.stopPropagation();
              if (window.confirm("Are you sure you want to delete this game permanently?")) {
                useVaultStore.getState().removeVaultGame(g);
              }
            }}
          >
      ...
      <div className="flex items-center justify-between text-[8px] font-mono mt-1">
        <span
          className="text-zinc-300 font-bold max-w-[120px] truncate"
          title={displayName}
        >
          {displayName}
        </span>
      ...
      <div className="grid grid-cols-2 gap-x-2 gap-y-0.5 mt-1 text-[9px] text-zinc-400 font-mono">
        <div className="flex justify-between hover:bg-white/5 px-1 py-0.5 rounded-sm">
          ...
          <span className="font-bold text-zinc-200">{g.lines_cleared}</span>
        </div>
        <div className="flex justify-between hover:bg-white/5 px-1 py-0.5 rounded-sm">
          <span className="text-zinc-500">STEPS</span>
          <span className="font-bold text-zinc-200">{g.steps.length}</span>
        </div>
        <div className="flex justify-between hover:bg-white/5 px-1 py-0.5 rounded-sm">
