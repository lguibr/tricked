...
fn atomic_add_float(atom: &AtomicU64, delta: f64) {
...
impl SegmentTree {
    ...
    pub fn update(&self, data_index: usize, priority_value: f64) {
        ...
        let mut old_val_bits = self.tree_array[tree_index].load(Ordering::Relaxed);
        let mut old_val = f64::from_bits(old_val_bits);
        loop {
            match self.tree_array[tree_index].compare_exchange_weak(
                old_val_bits,
                priority_value.to_bits(),
                Ordering::AcqRel,
                Ordering::Relaxed,
            ) {
                Ok(_) => break,
        ...
        tree_index /= 2;
        while tree_index > 0 {
            atomic_add_float(&self.tree_array[tree_index], delta_change);
            tree_index /= 2;
        }

        // Atomics reduce lock contention, but we'll trace the CAS operations overhead.
    ...
    pub fn update_batch(&self, updates: &[(usize, f64)]) {
        ...
        for &(data_index, priority_value) in updates {
            ...
            assert!(
                data_index < self.tree_buffer_capacity_limit,
                "Segment tree update index {} violates bounds!",
                data_index
            );
            ...
            if delta != 0.0 {
                tree_index /= 2;
                while tree_index > 0 {
                    atomic_add_float(&self.tree_array[tree_index], delta);
                    tree_index /= 2;
                }
            }
        }
