...
def test_overfit_single_batch():
    ...
    assert final_loss < initial_loss, "Model failed to overfit / reduce loss on a single synthetic batch!"
...
def test_overfit_complex_batch_and_hooks():
    ...
    for i in range(5):
        ...
        h = model.representation(states)
        scale_hook_called = []
        h.register_hook(lambda grad, h_called=scale_hook_called: h_called.append(True) or grad * 0.5)
        ...
        for k in range(unroll_steps):
            ...
            h.register_hook(lambda grad, uh_called=unrolled_h_called: uh_called.append(True) or grad * 0.5)
            loss += h.sum() + r.sum()
        ...
        assert len(unrolled_h_called) == unroll_steps, "Gradient scale hook for recurrent dynamics h did not fire!"
...
def test_ema_weights_update():
    ...
    for p_ema in ema_model.parameters():
        assert torch.allclose(p_ema.data, torch.full_like(p_ema.data, 0.01)), "EMA update incorrect!"
