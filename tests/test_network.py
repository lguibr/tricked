...
def test_scalar_support_bidirectionality() -> None:
    ...
    expected = scalars.sign() * (torch.sqrt(torch.abs(scalars) + 1.0) - 1.0) + model.epsilon * scalars
    expected = expected.clamp(-model.support_size, model.support_size)
    expected_recovered = torch.sign(expected) * ((((-1.0 + torch.sqrt(1.0 + 4.0 * model.epsilon * (1.0 + model.epsilon + expected.abs()))) / (2.0 * model.epsilon)) ** 2) - 1.0)
