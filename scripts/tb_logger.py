    ...
    for message in pubsub.listen():
        if message['type'] == 'message':
            ...
            if channel == 'tricked_config':
                ...
                get_writer().add_text("Metadata/Config", f"```json\n{config_str}\n```", step)

            elif channel == 'tricked_training':
                if "loss" in data:
                    get_writer().add_scalar("train/loss", data.get("loss"), step)
                if "policy_loss" in data:
                    get_writer().add_scalar("train/policy_loss", data.get("policy_loss"), step)
                if "value_loss" in data:
                    get_writer().add_scalar("train/value_loss", data.get("value_loss"), step)
                if "reward_loss" in data:
                    get_writer().add_scalar("train/reward_loss", data.get("reward_loss"), step)
                step += 1
            elif channel == 'tricked_games':
                if "score" in data:
                    get_writer().add_scalar("eval/score", data.get("score"), step)
                if "steps" in data:
                    get_writer().add_scalar("eval/steps", data.get("steps"), step)
            elif channel == 'tricked_metrics':
                if "value" in data and "name" in data:
                    metric_name = data.get("name")
                    if "/" not in metric_name:
                        metric_name = f"metrics/{metric_name}"
                    get_writer().add_scalar(metric_name, data.get("value"), step)
                    if step % 5 == 0:
                        get_writer().flush()
