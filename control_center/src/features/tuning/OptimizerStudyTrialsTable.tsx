        ...
        <table className="w-full text-xs text-left whitespace-nowrap border-collapse">
          <thead className="text-zinc-500 sticky top-0 z-20 bg-[#0f0f12]/90 backdrop-blur-md shadow-sm border-b border-white/5">
            <tr>
              ...
              <th className="px-6 py-4 font-black uppercase tracking-widest text-[9px] border-b border-white/[0.02]">
                Values <span className="text-zinc-600 font-normal lowercase">(HW / Loss)</span>
              </th>
          ...
          <tbody className="divide-y divide-white/[0.02]">
            {trials.slice().reverse().map((t) => (
              <tr key={t.number} className="hover:bg-white/[0.03] transition-colors group cursor-default">
                ...
                <td className="px-6 py-4 font-mono text-zinc-400">
                  {Array.isArray(t.value) && t.value.length >= 2 && t.value[0] != null && t.value[1] != null ? (
                    <div className="flex gap-2.5">
                      <span className="text-blue-400/90 font-bold">{Number(t.value[0]).toFixed(2)}</span>
                  ...
                  ) : Array.isArray(t.value) && t.value.length > 0 && t.value[0] != null ? (
