...
import { getPieceMask, getVectorRotatedMask, getGridBits, renderTriangle, u64Low, u64High } from "./PlaygroundMath";
...
export function TrickedPlayground() {
  ...
  const applyMove = async (slot: number) => {
    ...
    const ok = await applyMoveToStore(slot, u64Low(previewMask), u64High(previewMask), actionTaken, pid);
    if (ok) {
  ...
  const boardMask = useMemo(() => {
    ...
    return BigInt.asUintN(64, BigInt(gameState.board_low)) | (BigInt.asUintN(64, BigInt(gameState.board_high)) << 64n);
  }, [gameState]);
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col items-center justify-center relative">
          <div className="absolute top-4 right-4 flex gap-2 z-50">
            <Button variant="outline" size="icon" className="bg-zinc-950 border-zinc-800 shadow-xl" onClick={() => setBoardRotation((r) => r - 60)}>
              <RotateCcw className="w-5 h-5 text-zinc-400" />
            </Button>
            <Button variant="outline" size="icon" className="bg-zinc-950 border-zinc-800 shadow-xl" onClick={() => setBoardRotation((r) => r + 60)}>
              <RotateCw className="w-5 h-5 text-zinc-400" />
            </Button>
            <Button variant="outline" size="icon" className="bg-zinc-950 border-zinc-800 shadow-xl ml-4" onClick={() => setBoardRotation(0)} title="Reset Board Rotation">
              <RefreshCw className="w-5 h-5 text-zinc-400" />
            </Button>
            <Button variant="outline" size="sm" className="bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20 hover:text-red-300 shadow-xl ml-4 font-black tracking-widest uppercase" onClick={() => startGame()}>
              Restart
            </Button>
          </div>
          ...
          {!gameState && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 flex flex-col items-center justify-center p-12 bg-black/80 rounded-2xl border border-white/10 backdrop-blur-sm shadow-2xl">
              <h1 className="text-3xl font-black text-emerald-400 mb-6 tracking-widest uppercase">Play</h1>
              <Button variant="default" size="lg" onClick={() => startGame()} className="bg-emerald-500 hover:bg-emerald-400 text-black font-black uppercase tracking-widest px-12 py-6 text-lg hover:scale-105 transition-transform">
                Start Game
              </Button>
            </div>
          )}
          ...
          >
            <svg width="350" height="350" viewBox="-80 -70 160 140" className="overflow-visible filter drop-shadow-2xl">
              {(gridCoords as CellCoord[]).map((c) => {
                ...
                if (isActive) fillClass = "fill-zinc-800 stroke-zinc-900 drop-shadow-[0_0_5px_rgba(255,255,255,0.1)] hover:fill-zinc-700";
                if (isPreview) fillClass = "fill-emerald-500/80 stroke-emerald-400 drop-shadow-[0_0_10px_rgba(16,185,129,0.5)] cursor-pointer";
                return (
                  <g key={c.id}>
                    {renderTriangle(
                      c,
                      fillClass,
                      () => { if (selectedSlot !== null && previewMask !== null) applyMove(selectedSlot); },
