...
import { getVectorRotatedMask, getGridBits, renderTriangle } from "../playground/PlaygroundMath";
...
export function EvaluationDeployQueue({
  available,
}: {
  available: number[];
}) {
  const renderMiniPiece = (
    ...
    if (pieceId === -1) {
      return <div className="w-20 h-20 rounded-xl border border-dashed border-zinc-800/50 mx-auto" />;
    }
    const validMasks = [];
    for (let i = 0; i < 96; i++) {
    ...
    return (
      ...
      >
        <svg viewBox="-40 -40 80 80" className="w-full h-full pointer-events-none drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]">
          <g transform={`translate(${-cx}, ${-cy})`}>
            {rep.bits.map((b) => renderTriangle((gridCoords as CellCoord[])[b], "fill-emerald-500/40 stroke-emerald-400/80"))}
          </g>
        </svg>
      </div>
    );
  ...
  return (
    <div className="absolute right-6 top-1/2 -translate-y-1/2 w-32 border-l border-white/5 pl-4 flex flex-col justify-center gap-4">
      <h3 className="text-zinc-600 uppercase tracking-widest text-[9px] mb-2 font-black rotate-180" style={{ writingMode: 'vertical-rl' }}>
        Agent Buffer
      </h3>
      {available ? (
