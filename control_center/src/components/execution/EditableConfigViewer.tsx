...
export const ENGINE_PARAM_GROUPS: GroupDef[] = [
    {
        ...
        fields: [
            ...
            { key: "inference_batch_size_limit", label: "Inference Limit", min: 16, max: 1024, step: 16 },
        ]
    },
    {
        title: "MCTS Engine",
        color: "text-blue-400",
        fields: [
            { key: "simulations", label: "Simulations", min: 10, max: 5000, step: 10 },
            { key: "max_gumbel_k", label: "Max Gumbel K", min: 4, max: 64, step: 1 },
            { key: "inference_timeout_ms", label: "Inference Timeout (ms)", min: 10, max: 1000, step: 10 },
        ]
    },
    {
        ...
        fields: [
            ...
            { key: "temporal_difference_steps", label: "TD Steps", min: 1, max: 15, step: 1 },
            { key: "buffer_capacity_limit", label: "Buffer Capacity", min: 10000, max: 1000000, step: 10000 },
        ]
    },
    {
        ...
        fields: [
            ...
            { key: "support_size", label: "Value Support Size", min: 10, max: 1000, step: 10 },
        ]
    }
];
...
export function EditableConfigViewer({ run }: { run: Run }) {
    ...
    return (
        <div className="relative group/config mt-1 bg-[#0a0a0c]/50 border border-border/20 rounded-md p-3 flex flex-col gap-3 max-h-[400px] overflow-y-auto">
            ...
            <ParameterForm mode="single" value={config} onChange={handleChange} groups={ENGINE_PARAM_GROUPS} />
        </div>
    );
}
