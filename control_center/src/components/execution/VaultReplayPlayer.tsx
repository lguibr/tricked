...
export function VaultReplayPlayer() {
  ...
  const activePlacementCells = useMemo(() => {
    if (!placementMask) return [];
    return getGridBits(placementMask);
  }, [placementMask]);
  ...
  const renderMiniPiece = (pieceId: number, isBeingPlaced: boolean) => {
    if (pieceId === -1) {
      return (
        <div className="w-16 h-16 rounded-xl border border-dashed border-white/5 mx-auto" />
      );
    }
    
    // Find the first valid mask to sample relative geometry to center it
    const validMasks = [];
    for (let i = 0; i < 96; i++) {
    ...
    if (!rep) return <div className="w-16 h-16 rounded-xl border border-dashed border-red-500/20 mx-auto" />;
      ...
      <div className={`w-16 h-16 rounded-xl mx-auto overflow-hidden ${isBeingPlaced ? "bg-emerald-500/20 border border-emerald-500/50 scale-110 shadow-[0_0_15px_rgba(16,185,129,0.3)] transition-all" : "bg-[#080808] border border-white/5 opacity-50 transition-all"}`}>
        <svg viewBox="-40 -40 80 80" className="w-full h-full">
          <g transform={`translate(${-cx}, ${-cy})`}>
            {rep.bits.map((b) => renderTriangle((gridCoords as CellCoord[])[b], "fill-emerald-500/80 stroke-emerald-950"))}
          </g>
        </svg>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0a0a] border-l border-white/5">
      ...
      <div className="p-4 border-b border-white/5 flex items-center justify-between bg-black/40">
         <div>
            <h2 className="text-sm font-black text-white tracking-widest uppercase mb-1">Vault Game Replay</h2>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <div className="shadow-[0_0_120px_rgba(0,0,0,0.5)] rounded-full p-4 bg-black/40 border border-white/5 flex items-center justify-center">
                ...
                >
                  {(gridCoords as CellCoord[]).map((c) => {
                    ...
                    const isPlacedHere = activePlacementCells.includes(c.id);
                    ...
                    if (isPlacedHere) {
                      fillClass = "fill-emerald-400 stroke-emerald-900 drop-shadow-[0_0_8px_rgba(16,185,129,0.5)] z-10 relative";
                    }
        ...
        <div className="w-48 border-l border-white/5 bg-[#080808] flex flex-col">
           ...
           <div className="flex flex-col gap-6 p-4 justify-center flex-1">
              {stepData?.available.map((pid, idx) => (
                  <div key={idx}>
                      {renderMiniPiece(pid, pid !== -1 && pid === stepData.piece_identifier)}
                  </div>
              ))}
           </div>
        </div>
      ...
      <div className="h-24 border-t border-white/5 bg-black p-4 flex flex-col gap-3 shrink-0">
          <div className="flex-1 flex items-center gap-4">
             <div className="flex gap-1 shrink-0">
                ...
                >
                   {isPlaying ? <VscDebugPause className="text-emerald-400 w-4 h-4"/> : <VscPlay className="text-emerald-400 w-4 h-4"/>}
                </Button>
                ...
                <Button 
                    ...
                    size="icon" 
                    className="h-8 w-8 bg-white/5 border-white/10"
                    onClick={() => setCurrentStep(prev => Math.min(game.steps.length - 1, prev + 1))}
             ...
             <div className="text-[10px] uppercase font-mono font-bold text-zinc-500 w-12 text-right">
                {currentStep + 1}<span className="opacity-50">/{game.steps.length}</span>
             </div>
          </div>
      </div>
    </div>
