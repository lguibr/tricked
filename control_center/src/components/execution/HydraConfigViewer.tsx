    ...
    return (
      <div className="mb-4 last:mb-0">
        ...
        <div className="flex flex-wrap gap-2">
          {items.map(([key, value]) => (
            <ConfigBadge key={key} paramKey={key} value={value} groupTitle={title} compact={false} />
          ))}
        </div>
      </div>
    );
...
export function CompactTrialParams({ params }: { params: Record<string, any> }) {
  ...
  const renderCompactGroup = (title: string, items: [string, any][]) => {
    ...
    return (
      <>
        {items.map(([key, value]) => (
          <ConfigBadge key={key} paramKey={key} value={value} groupTitle={title} compact={true} />
        ))}
      </>
    );
  };
    ...
    <TooltipProvider>
      <div className="flex items-start gap-2">
        ...
        <div className="flex flex-col gap-1 mt-0.5 shrink-0">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon" onClick={handleCopy} className="h-5 w-5 hover:bg-white/10">
