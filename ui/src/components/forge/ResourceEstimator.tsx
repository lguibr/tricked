...
export function ResourceEstimator({ d_model, num_blocks, batch_size }: EstimatorProps) {
  ...
  const isCritical = estimatedGB > 15;

  let stateColor = 'text-green-400';
  let Icon = CheckCircle2;
  let message = 'Fits comfortably in 16GB mobile GPUs.';

  if (isCritical) {
    stateColor = 'text-red-500';
    Icon = AlertCircle;
    message = 'Exceeds 16GB limits. Will OOM.';
  } else if (isWarning) {
    stateColor = 'text-yellow-400';
    Icon = Zap;
    message = 'Nearing 16GB limits. Danger zone.';
  }
