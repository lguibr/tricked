  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-4rem)] gap-8 p-8 max-w-7xl mx-auto w-full pb-20 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="lg:col-span-2 flex flex-col gap-8">

          <motion.div variants={itemVariants} className="bg-muted border border-border p-6 rounded-2xl shadow-xl backdrop-blur-xl">
            ...
            <div className="space-y-8">
              <div className="grid gap-2">
                ...
                <Slider min={32} max={1024} step={32} value={[config.d_model]} onValueChange={(v: any) => update('d_model', v[0])} />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={2} max={48} step={2} value={[config.num_blocks]} onValueChange={(v: any) => update('num_blocks', v[0])} />
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-muted border border-border p-6 rounded-2xl shadow-xl backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
              <LogarithmicSlider label="Simulations" min={16} max={4096} value={config.simulations} onChange={(v) => update('simulations', Math.round(v))} />
              <LogarithmicSlider label="Buffer Capacity" min={1000} max={1000000} value={config.capacity} onChange={(v) => update('capacity', Math.round(v))} />
              ...
              <div className="grid gap-2">
                ...
                <Slider min={1} max={20} step={1} value={[config.unroll_steps]} onValueChange={(v: any) => update('unroll_steps', v[0])} />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={1} max={30} step={1} value={[config.td_steps]} onValueChange={(v: any) => update('td_steps', v[0])} />
              </div>

              <div className="grid gap-2">
                <Label className="text-sm font-semibold text-muted-foreground">Network Port (ZMQ)</Label>
                <Input className="font-mono bg-background/50 border-border text-muted-foreground" value="tcp://127.0.0.1:5555" readOnly disabled />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={1} max={256} step={1} value={[config.zmq_batch_size]} onValueChange={(v: any) => update('zmq_batch_size', v[0])} />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={1} max={50} step={1} value={[config.zmq_timeout_ms]} onValueChange={(v: any) => update('zmq_timeout_ms', v[0])} />
              </div>
              ...
              <div className="grid gap-2">
                ...
                <Slider min={2} max={32} step={1} value={[config.max_gumbel_k]} onValueChange={(v: any) => update('max_gumbel_k', v[0])} />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={0.1} max={5.0} step={0.1} value={[config.gumbel_scale]} onValueChange={(v: any) => update('gumbel_scale', v[0])} />
              </div>
          ...
          <motion.div variants={itemVariants} className="bg-muted border border-border p-6 rounded-2xl shadow-xl backdrop-blur-xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8">
              ...
              <div className="grid gap-2 col-span-1 md:col-span-2">
                ...
                <Input className="bg-background/50 border-border" value={config.exp_name} onChange={(e) => update('exp_name', e.target.value)} />
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="flex flex-col gap-6 lg:sticky lg:top-24 self-start">
          <ResourceEstimator d_model={config.d_model} num_blocks={config.num_blocks} batch_size={config.train_batch_size || 1024} />
          ...
          <div className="bg-muted border border-border p-6 rounded-2xl shadow-xl backdrop-blur-xl">
            ...
            <div className="space-y-8">
              <LogarithmicSlider label="Learning Rate" min={1e-5} max={1e-2} value={config.lr_init} onChange={(v) => update('lr_init', v)} />
              <LogarithmicSlider label="Batch Size" min={64} max={16384} value={config.train_batch_size} onChange={(v) => update('train_batch_size', Math.round(v))} />
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={1} max={50} step={1} value={[config.train_epochs]} onValueChange={(v: any) => update('train_epochs', v[0])} />
              </div>
              <div className="grid gap-2">
                <div className="flex justify-between items-center">
                ...
                <Slider min={1} max={128} step={1} value={[config.num_processes]} onValueChange={(v: any) => update('num_processes', v[0])} />
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 bg-black/40 border border-primary/20 rounded-none p-6 relative shadow-[0_0_30px_rgba(0,251,251,0.1)] shrink-0 flex flex-col group">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (el) {
                    ...
                    setTimeout(() => el.innerHTML = '<rect width="14" height="14" x="8" y="8" rx="2" ry="2"/><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2"/>', 2000); // Copy
                  }
                }}
              >
                <svg id="copy-icon" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="14" height="14" x="8" y="8" rx="2" ry="2" /><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" /></svg>
            ...
            <pre className="text-xs font-mono text-muted-foreground bg-background/50 p-4 rounded-none border border-white/5 overflow-x-auto whitespace-pre-wrap flex-1 overflow-y-auto custom-scrollbar">
              {JSON.stringify(config, null, 2).split('\n').map((line, i) => (
                <div key={i} className="leading-snug"><span className="text-white/20 select-none mr-4 pr-4 border-r border-white/10 inline-block w-8 text-right">{(i + 1).toString()}</span><span className={line.includes(':') ? 'text-accent' : 'text-primary'}>{line}</span></div>
              ))}
            </pre>
          </div>
        </div>
