...
>
	<div class="flex items-center gap-2">
		<span
			class="font-headline text-xs font-bold text-on-surface-variant uppercase tracking-widest mr-2"
			>Complexity:</span
		>
		...
		<button
			...
			1
				? 'bg-primary border border-primary text-on-primary drop-shadow-[0_0_40px_var(--color-surface-tint)]'
				: 'bg-surface-container text-primary border border-outline-variant/30 hover:bg-primary/20 hover:text-primary hover:border-primary/50'}"
			>1 - Easy</button
		>
		...
		<button
			...
			class="px-5 py-2 text-xs font-headline rounded-none font-bold uppercase transition-all disabled:opacity-50 {engine.currentDifficulty ===
			6
				? 'bg-secondary border border-secondary text-surface drop-shadow-[0_0_10px_var(--color-secondary)]'
				: 'bg-surface-container text-secondary border border-outline-variant/30 hover:bg-secondary/20 hover:text-secondary hover:border-secondary/50'}"
			>6 - Master</button
		>
	</div>
	...
	<div class="flex items-center gap-3">
		...
		<button
			...
			class="flex items-center gap-2 px-6 py-2 text-xs font-headline rounded-none font-bold uppercase tracking-widest transition-all disabled:opacity-50 {engine.isTraining
				...
				: 'bg-secondary border border-secondary text-surface hover:scale-[0.98] drop-shadow-[0_0_10px_var(--color-secondary)]'}"
		>
			<span class="material-symbols-outlined text-sm" style="font-variation-settings: 'FILL' 1;"
				>{engine.isTraining ? 'pause' : 'play_arrow'}</span
			>
