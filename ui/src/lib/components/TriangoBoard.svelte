        ...
        <div class="absolute bottom-0 left-0 w-full bg-surface-dim/90 backdrop-blur-xl border border-outline-variant/20 border-x-0 border-b-0 p-6 z-20">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center gap-4">
                    <div class="px-3 py-1 bg-primary/10 border border-primary/20 text-primary font-headline text-[10px] tracking-widest uppercase font-bold rounded-none">Simulation Mode</div>
                    <span class="font-headline text-sm font-bold tracking-tighter text-on-surface uppercase flex items-center gap-3">
                        <span class="material-symbols-outlined text-outline text-lg">history</span>
                        Step Counter: {engine.replayStats.currentStep} / <span class="text-outline">{engine.replayStats.maxStep}</span>
                    </span>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <button onclick={() => engine.stopReplay()} class="px-5 py-2 rounded-none bg-error/10 text-error font-headline font-bold text-[10px] uppercase tracking-widest hover:bg-error hover:text-on-error border border-error/30 hover:border-error transition-colors">EXIT REPLAY</button>
                </div>
            </div>
            <div class="space-y-2">
                <div class="flex justify-between text-[10px] font-headline text-on-surface-variant font-bold uppercase tracking-widest">
                    <span>Playback Speed</span>
                    <span class="text-primary">{Math.round(1000 / engine.replaySpeedMs)} ops/sec</span>
                </div>
                <div class="relative h-1 bg-surface-container rounded-none overflow-visible border border-outline-variant/20">
                    <input
						type="range"
						min="50"
						max="1500"
	...
	{#if engine.gameState?.terminal && !engine.isReplaying && !engine.isLeaderboardOpen}
		<div class="absolute inset-0 bg-surface/90 flex flex-col items-center justify-center p-12 text-center pointer-events-auto z-20">
			<div class="mb-4 flex items-center gap-2 text-secondary">
				<span class="material-symbols-outlined text-4xl animate-pulse" style="font-variation-settings: 'FILL' 1;">warning</span>
			...
			<div class="grid grid-cols-2 gap-4 w-full max-w-md mb-8">
				<div class="p-4 rounded-none bg-surface-container-highest border border-outline-variant/30 border-l-4 border-l-secondary text-left drop-shadow-[0_0_40px_var(--color-surface-tint)]">
					<div class="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest mb-1">Final Efficiency</div>
					<div class="font-headline font-bold text-2xl text-secondary">{engine.gameState.score}</div>
				</div>
				<div class="p-4 rounded-none bg-surface-container-highest border border-outline-variant/30 border-l-4 border-l-primary text-left drop-shadow-[0_0_40px_var(--color-surface-tint)]">
					<div class="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest mb-1">Entities Cleared</div>
					<div class="font-headline font-bold text-2xl text-primary">{96 - (engine.gameState.pieces_left || 0)}/96</div>
				</div>
			</div>
			
			{#if !engine.isTraining}
				<button onclick={() => engine.resetGame(engine.currentDifficulty)} class="px-10 py-4 rounded-none bg-secondary/10 border border-secondary text-secondary font-bold font-headline uppercase tracking-widest text-sm hover:bg-secondary hover:text-surface transition-all hover:scale-105 drop-shadow-[0_0_15px_var(--color-secondary)]">Relaunch Engine</button>
			{:else}
				<div class="px-8 py-3 rounded-none border border-secondary/30 text-secondary font-bold uppercase tracking-widest text-xs animate-pulse bg-secondary/10">Self-Optimizing Loop...</div>
			{/if}
		</div>
	{/if}
</div>
